...
import { closest } from '@cloudpivot_v2/common/src/utils/dom';

import { WorkflowCenterRouter, CalculateMaxHeightTypes } from '../typings/workflow-center';
  ...
  commonHandler(res: any, type?: string) {
    ...
    if (res.errcode === 0) {
      ...
      if (res.data.content.length <= 0) {
        ...
        let calType: string = '';
        switch (name) {
          case WorkflowCenterRouter.MyUnfinishedWorkItem:
            calType = CalculateMaxHeightTypes.OnlyTable;
            break;
          case WorkflowCenterRouter.MyUnReadWorkItem:
            calType = CalculateMaxHeightTypes.HasBtns;
            break;
          case WorkflowCenterRouter.MyFinishedWorkItem:
            calType = CalculateMaxHeightTypes.HasFilters;
            break;
          case WorkflowCenterRouter.MyReadWorkItem:
            calType = CalculateMaxHeightTypes.HasFilters;
            break;
          case WorkflowCenterRouter.MyWorkflow:
            calType = CalculateMaxHeightTypes.HasTabsAndFilters;
            break;
          case WorkflowCenterRouter.QueryInstance:
            calType = CalculateMaxHeightTypes.HasTabsAndFilters;
            break;
          case WorkflowCenterRouter.QueryParticipantWorkItem:
            calType = CalculateMaxHeightTypes.HasTabsAndFilters;
            break;
          default: break;
        }
