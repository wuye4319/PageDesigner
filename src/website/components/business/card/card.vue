<template>
  <div class="myCard">
    <a-list class="myList" :grid="column" :dataSource="listData">
      <a-list-item
        slot="renderItem"
        slot-scope="item"
      ...
      >
        <a-card hoverable>
          <img
            ...
            slot="cover"
          />
          <template v-if="compAttr.actions" class="ant-card-actions" slot="actions">
            <a-icon v-for="(action,i) in item.actions" :type="action" :key="action + i" />
          </template>
          <a-card-meta>
            <h5 class="overElip" :title="item.title" v-if="compAttr.title" slot="title">{{ item.title }}</h5>
            <p class="desc overElip" :title="item.description" v-if="compAttr.description" slot="title">{{ item.description }}</p>
            <a-avatar
