<template>
  <div class="collapse" v-if="listData.length > 0" :class="{'arrow-right': pageData.arrow}">
    <a-collapse v-model="activeKey" :bordered="bordered" :accordion="!pageData.accordion">
      <a-collapse-panel 
...
export default class CollapseView extends Vue {
  ...
  get bordered() {
    let bordered = this.pageData.style === 'one' ? true : false;
    return bordered
  }
