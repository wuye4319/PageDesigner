<template>
  <div class="datamap">
    <div class="list">
      <span>数据对象</span>
      <a-select style="width: 50%;" :open="false" @dropdownVisibleChange="showModal" v-model="tableName" :disabled="dataModel.tableNameSwitch === 0">
        <a-select-option v-for="menu of tablesData" :key="menu.tableName" :value="menu.tableName">{{ menu.title }}</a-select-option>
      </a-select>
    </div>
    ...
    <a-table class="my-table" v-if="dataModel.mapDataSwitch !== 0" :columns="detailsColumns" :dataSource="detailsData" bordered :pagination="false">
      <template slot="val" slot-scope="val,obj,index">
        <div class="my-select" ref="my-select">
          ...
          >
            <a-select-option v-for="menu of compSelectList" :key="menu" :value="menu">{{ menu }}</a-select-option>
          </a-select>
        </div>
      </template>
    </a-table>
      ...
      <div class="data-list">
        <div class="menu">
          ...
          <template v-if="tablesData.length > 0">
            <a-menu style="width: 256px" class="scroll-style" mode="vertical" v-model="selectedKeys" @select="menuSelect">
        ...
        <div class="menu-cont">
          ...
          <a-table :columns="modalColumns" :dataSource="modalData" bordered :pagination="false" :scroll="{ y: 300 }" />
        </div>
      </div>
    </a-modal>
  </div>
...
export default class DataMap extends Vue {
  ...
  get detailsData() { // 数据映射选择
    let arr = [];
    let mapData = this.compData.dataModel.mapData
    if (this.tableName === this.selectedKeys[0]) {
  ...
  // 组件绑定数据
  compChange(index) {
    this.compData.dataModel.mapData[index].tableMap = this.detailsData[index]['val'];
  }
